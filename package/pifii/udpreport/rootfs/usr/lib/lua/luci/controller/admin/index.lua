...
function ApiWifinameSet( args )
	...
	if pass == "N/A" then
      tempV = "uci set wireless.@wifi-iface[0].encryption=none"
      luci.sys.call( tempV )
      tempV = "uci delete wireless.@wifi-iface[0].key"
      luci.sys.call( tempV )
    else
        tempV = "uci set wireless.@wifi-iface[0].encryption=psk+psk2"
        luci.sys.call( tempV )
        tempV = "uci set wireless.@wifi-iface[0].key=" .. pass
        luci.sys.call( tempV )
    end

    tempV = "uci set wireless.@wifi-iface[0].hidden=" .. hidden
    luci.sys.call( tempV )
...
function Api_wip_set(args)
    ...
    local wifimode = uci:get( "network", "wan", "_wantype"  ) or "wifi"
